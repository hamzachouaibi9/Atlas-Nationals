...
public class OverduePage extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.fragment_overdue_books);

        Toolbar toolbar = (Toolbar) findViewById(R.id.toolbar);
        setSupportActionBar(toolbar);
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        getSupportActionBar().setTitle("Overdue Books");


        //      find all of firebase data and initialize it with recyclerview
        auth = FirebaseAuth.getInstance();
        user_id = auth.getCurrentUser().getUid();

        databaseReference = FirebaseDatabase.getInstance().getReference().child("Overdue_Books").child(user_id);

        list = (RecyclerView) findViewById(R.id.over_due_list);
        list.hasFixedSize();
        list.setLayoutManager(new LinearLayoutManager(this));

        progressDialog = new ProgressDialog(this);
        progressDialog.setMessage("Loading Over Due Books");
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                if (dataSnapshot.exists()){
                    FirebaseRecyclerAdapter<Book, BooksViewHolder> firebaseRecyclerAdapter = new FirebaseRecyclerAdapter<Book, BooksViewHolder>
                            (Book.class, R.layout.book_row, BooksViewHolder.class, databaseReference) {
                        @Override
                        protected void populateViewHolder(BooksViewHolder viewHolder, Book model, int position) {
//                            populate the view
                            viewHolder.title.setText(model.getTitle());
                            viewHolder.description.setText(R.string.overdue);
                            viewHolder.title.setTextColor(Color.parseColor("#000000"));
                            viewHolder.title.setTextSize(25);
                            Picasso.with(OverduePage.this).load(model.getImage()).into(viewHolder.image);
                            viewHolder.description.setTextColor(Color.parseColor("#8e0000"));
                    ...
                    progressDialog.dismiss();
                    Toast.makeText(OverduePage.this, "You Have No Overdue Books", Toast.LENGTH_SHORT).show();
                }
            }
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        actionView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                startActivity(new Intent(OverduePage.this, CartPage.class));
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.help_button) {
            startActivity(new Intent(OverduePage.this, HelpHome.class));
            overridePendingTransition(R.anim.fadein, R.anim.fadeout);
        }
        ...
        if (id == R.id.nav_account){
            startActivity(new Intent(OverduePage.this, AccountPage.class));
            overridePendingTransition(R.anim.fadein, R.anim.fadeout);
        }
        ...
        if (id == R.id.app_bar_sign_out) {
            ...
            mGoogleSignInClient.signOut().addOnCompleteListener(new OnCompleteListener<Void>() {
                ...
                public void onComplete(@NonNull Task<Void> task) {
                    startActivity(new Intent(OverduePage.this, Login.class));
                    finish();
                }
            });
            return true;
        ...
        if (id == R.id.app_bar_search){
            startActivity(new Intent(OverduePage.this, SearchPage.class));
            overridePendingTransition(R.anim.fadein, R.anim.fadeout);
            return true;
        }
